...
private slots:
    void OnStartGame();
    void OnExportRecompiledSource(const QString& output_dir, bool source_only = false);
    void OnRestartGame();
    void OnPauseGame();
...
private:
    ...
    // Last perf sample, kept so the MCP state tool can report it.
    // GetAndResetPerfStats() clears the counters, so whoever calls it takes the
